...
void conv2d::load_input()
{
	...
	// std::cout<<"total_filters_chunks="<<total_filters_chunks<<std::endl;
	// std::cout<<"total_input_chunks="<<total_input_chunks<<std::endl;
	// std::cout<<"max_cacheable_rows="<<max_cacheable_rows<<std::endl;
	// std::cout<<"max_cacheable_rows_init="<<max_cacheable_rows_init<<std::endl;
	// std::cout<<"max_cacheable_filters="<<max_cacheable_filters<<std::endl;
	// std::cout<<"max_cacheable_bias_chunks="<<max_cacheable_bias_chunks<<std::endl;
	// std::cout<<"out_channel_offset_incr="<<out_channel_offset_incr<<std::endl;
	// std::cout<<"out_channel_pool_offset_incr="<<out_channel_pool_offset_incr<<std::endl;
	// std::cout<<"feature_offset_start_base="<<feature_offset_start_base<<std::endl;
	// std::cout<<"loadable_chan="<<loadable_chan<<std::endl;
	// std::cout<<"chan_iters="<<chan_iters<<std::endl;
	// std::cout<<"chan_rem="<<chan_rem<<std::endl;
	// std::cout<<"loadable_chan_sz="<<loadable_chan_sz<<std::endl;
	// std::cout<<"chan_rem_sz="<<chan_rem_sz<<std::endl;
	     ...
	     filter_chunk++)
	{
		...
		if (!(!i && misaligned)) {
			...
			if (i + 1 < adj_words_to_load)
				...
				if(plm_weights_index>=WEIGHTS_PLM_SIZE){
					std::cout<<"plm_w out of bound!!!"<<std::endl;
					exit(0);
				}
#endif
	    }
		...
		for (uint16_t i = 0; i < adj_words_to_load; i += DMA_WORD_PER_BEAT)
		{
			...
			if (ping_bias) {
				...
				plm_bias_pong[plm_bias_i++] =
					...
					if(plm_bias_i>=BIAS_PLM_SIZE){
						std::cout<<"plm_bias out of bound!!!"<<std::endl;
						exit(0);
					}
#endif
		}
				...
				if (!(!i && misaligned)) {
					...
					if (i + 1 < adj_words_to_load)
						...
						dataBv.range(63, 32).to_int();
								...
								if(plm_in_index>=INPUT_PLM_SIZE){
									std::cout<<"plm_in out of bound!!!"<<std::endl;
									exit(0);
								}
#endif
			    }
...
void conv2d::store_output()
{
	...
	for (uint16_t b = 0; b < batch_size; b++)
	{
		...
		for (uint16_t filter_i = 0; filter_i < cached_filters; filter_i++)
		{
			...
			FPDATA mean, var, b_w, b_b, res_conv, res_batch;
			if(filter_i==0){
				mean = -0.5719402;
				// var = 0.2857622613292385;
				...
				mean = -0.7738392;
				// var = 0.37640823582913274;
				var = 0.14167316 + 0.00001;
				b_w = 1.3536469;
				b_b = -0.7578076;
			}
		    uint16_t batchNorm_base = filter_i *
			    round_up(rows_to_load_adj * output_w, DMA_WORD_PER_BEAT);
			for (uint16_t i = 0; i < output_w*output_w; i++){
				if (ping_output) {
					res_conv = INT2FP(plm_out_ping[batchNorm_base]);
				} else {
					res_conv = INT2FP(plm_out_pong[batchNorm_base]);
				}
				
				res_batch = (res_conv-mean)/sqrt(var) * b_w + b_b;
				...
				if (ping_output) {
					plm_out_ping[batchNorm_base++] = FP2INT(res_batch);
				} else {
					plm_out_pong[batchNorm_base++] = FP2INT(res_batch);
				}
			}
		    {
			HLS_DEFINE_PROTOCOL();
		...
		for (uint12_t in_i = 0; in_i < chan_iters; in_i++)
		{
			...
			for (uint16_t out_c = 0; out_c < (output_w << ilog2(stride)); out_c += stride)
			{
				...
				for (uint16_t j = 0; j < PARALLELISM; j++) {
					...
					// std::cout<<"lala:"<<in_r<<","<<in_c<<","<<ch<<std::endl;
					// std::cout<<"lala:"<<loadable_rows<<","<<width<<","<<channels<<std::endl;
					// std::cout<<"in_r_base:"<<in_r_base<<endl;
					// std::cout<<"in_c_base:"<<in_c_base<<endl;
					// std::cout<<"ch_base:"<<ch_base<<endl;
					// std::cout<<INT2FP(val)<<","<<INT2FP(plm_in_ping[plm_i])<<std::endl;
				...
				for (uint16_t f = 0; f < loadable_filters; f++) {	
					...
					FPDATA res_batch, res_batch1, res_batch2, res_batch3 = 0;
					...
					// fprintf(stderr,"w0=%f, w1=%f, w2=%f, w3=%f, w4=%f, w5=%f, w6=%f, w7=%f, f=%d\n", 
					// (float) reg_w[0], (float) reg_w[1], (float) reg_w[2], (float) reg_w[3], 
					// (float) reg_w[4], (float) reg_w[5], (float) reg_w[6], (float) reg_w[7],
					// f);
					// fprintf(stderr,"p0=%f, p1=%f, p2=%f, p3=%f, p4=%f, p5=%f, p6=%f, p7=%f, f=%d\n", 
					// (float) reg_patch[0], (float) reg_patch[1], (float) reg_patch[2], (float) reg_patch[3], 
					// (float) reg_patch[4], (float) reg_patch[5], (float) reg_patch[6], (float) reg_patch[7],
					// f);
					...
					((reg_mac[4] + reg_mac[5]) +
					 (reg_mac[6] + reg_mac[7])) +
					res_partial;
					// fprintf(stderr,"m0=%f, m1=%f, m2=%f, m3=%f, m4=%f, m5=%f, m6=%f, m7=%f, ,partial=%f, res_mac=%f\n", 
					// (float) reg_mac[0], (float) reg_mac[1], (float) reg_mac[2], (float) reg_mac[3], 
					// (float) reg_mac[4], (float) reg_mac[5], (float) reg_mac[6], (float) reg_mac[7],
					// (float)res_partial, (float) res_mac);
					// fprintf(stderr, "--------------------------------------------------\n");
				    ...
				    // }
					res_relu = res_mac;
					// std::cout<<"f="<<f<<", cov="<<res_relu<<", batch=";
					// res_batch = (res_relu-mean)/var * b_w + b_b;
					// fprintf(stderr,"batch0-res_relu=%f\n", (float) res_relu);
					// res_batch1 = (res_relu-mean);
					// fprintf(stderr,"batch1-res_relu=%f\n", (float) res_batch1);
					// res_batch2 = res_batch1/var;
					// fprintf(stderr,"batch2-res_relu=%f\n", (float) res_batch2);
					// res_batch3 = res_batch2 * b_w;
					// fprintf(stderr,"batch3-res_relu=%f\n", (float) res_batch3);
					// res_batch = res_batch3 + b_b;
					// res_batch = res_relu;
					// fprintf(stderr, "f=%d, conv=%f, batch1=%f, batch2=%f, batch3=%f, batch=%f\n", 
					// f, (float) res_relu, (float) res_batch1, (float) res_batch2,(float) res_batch3,(float) res_batch);
					// std::cout<<res_batch<<std::endl;
					...
					if(out_plm_addr >= OUTPUT_PLM_SIZE){
						std::cout<<"plm_out out of bound!!!"<<std::endl;
						exit(0);
					}
