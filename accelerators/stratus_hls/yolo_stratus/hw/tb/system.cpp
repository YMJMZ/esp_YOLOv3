...
void system_t::load_memory()
{
    ...
    fprintf(stderr, "DMA_WORD_PER_BEAT=%d\n", DMA_WORD_PER_BEAT);
    fprintf(stderr, "in_words_adj=%d\n", in_words_adj);
    ...
    // ESP_REPORT_INFO("start INIT_TENSOR");
    // fprintf(stderr, "input_size=%d\n", in_size);
    // ifstream in("/home/esp2022/sz2949/testbench/channel_01_float/input_tensor.bin", ios::in | ios::binary);
    // in.read((char *) input, in_size * sizeof(float));
    // fprintf(stderr, "size = %d\n", in.gcount()/4);
    // in.close();
    // ESP_REPORT_INFO("INPUT INIT DONE");
    // fprintf(stderr, "weight_size=%d\n", weights_size);
    // in.open("/home/esp2022/sz2949/testbench/channel_01_float/conv_filters.bin", ios::in | ios::binary);
    // in.read((char *) weights, weights_size * sizeof(float));
    // fprintf(stderr, "size = %d\n", in.gcount()/4);
    // in.close();
    // fprintf(stderr, "\n");
    // // init_tensor(bias, bias_size, "/home/esp2022/sz2949/testbench/channel_0_float/input_tensor.bin");
    // ESP_REPORT_INFO("W INIT DONE");

    init_tensor(input, in_size, true);
    init_tensor(weights, weights_size, true);
    init_tensor(bias, bias_size, true);

    // in.open("/home/esp2022/sz2949/testbench/channel_01_float/convoluted.bin", ios::in | ios::binary);
    // in.read((char *) sw_output, 2 * 173056 * sizeof(float));
    // fprintf(stderr, "size = %d\n", in.gcount()/4);
    // in.close();
    // ESP_REPORT_INFO("GT INIT DONE");
...
int system_t::validate()
{
    ...
    ofstream out("/home/esp2022/sz2949/testbench/channel_01_float/convoluted_hw.bin", ios::binary);
    for(int i=0; i<10; i++){
        fprintf(stderr, "%f, ", hw_output[i]);
    }
    fprintf(stderr, "\n");
    out.write((char *) hw_output, batch_size * num_filters * output_pool_h * output_pool_w * sizeof(float));
    fprintf(stderr, "hw_output_size = %d\n", batch_size * num_filters * output_pool_h * output_pool_w);
